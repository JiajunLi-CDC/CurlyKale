...
namespace CurlyKale
{
    public class HeatGeodesics
    {
        ...
        public double[] u;
        /// <summary>
        /// Normalized gradient of heat field, stored on Faces.
        /// </summary>
        public Vector3d[] X;
        /// <summary>
        ...
        public double[] phi;
        /// <summary>
        /// Gradient of distance field, stored on Faces.
        /// </summary>
        public Vector3d[] GradPhi;
        ...
        /// </summary>
        public void CalculateGeodesics(Vertex source, bool additional)
        {
            if (!additional)
            {
                s.Clear();
        ...
        /// </summary>
        public void CalculateGeodesics(IEnumerable<Vertex> sources, bool additional)
        {
            if (!additional)
            {
                s.Clear();
        ...
        public void CalculateGeodesics()
        {
            ...
            {
                Face face = g.faces[i];
                Vector3d normal = face.CalculateNormalTri();
                face.FillEdgeArray();
                double gx = 0, gy = 0, gz = 0;
                foreach (Halfedge edge in face.edges)
                {
                    Vector3d temp = Vector3d.CrossProduct(normal, edge.prev.vertex.p - edge.next.vertex.p);
                ...
                {
                    X[i] = new Vector3d((float)(gx / l), (float)(gy / l), (float)(gz / l));
                }
                face.ClearEdgeArray();
            }
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                foreach (Halfedge edge in g.vertices[i].edges)
                {
                    if (edge.face.index != -1)
                    {
                        ...
                        divX[i] += Vector3d.Multiply(div[3 * edge.face.index + (edgeIndex + 2) % 3] - div[3 * edge.face.index + (edgeIndex + 1) % 3], X[edge.face.index]);
                    }
                }
                divXmean += divX[i];
            }
            ...
            for (int i = 0; i < f; i++)
            {
                ...
                double l = System.Math.Sqrt(gx * gx + gy * gy + gz * gz);
                GradPhi[i] = new Vector3d((float)(gx / l), (float)(gy / l), (float)(gz / l));
                face.ClearEdgeArray();
            }
            ...
            Vector2d[] uv = new Vector2d[n];
            //Vector4[] tangents = new Vector4[n];
            for (int i = 0; i < n; i++)
            {
                uv[i] = new Vector2d((float)(phi[i] / Settings.mappingDistance), 0);
                Vector3d tgt = new Vector3d();
